    	...
    	switch (item.getItemId()) {
	      ...
	      case R.id.menu_share_new:
	    	  if (error || data.length()<10)
	    		  break;
			  String shareBody = getResources().getString(R.string.share_money_1)
			...
			loadM = new AsyncTask<Void, Void, Void>() {
                ...
                protected Void doInBackground(Void... params) {
                    try {
                    	...
                    	Document doc = Jsoup.connect("http://www.horoscope.com/us/horoscopes/money/horoscope"+getResources().getStringArray(R.array.nameOfHoroscopecForLoadHoroscopeCom)[UNIVERSAL_ID]+".aspx?sign="+getResources().getStringArray(R.array.nameOfzodiacForLoadHoroscopeCom)[Integer.parseInt(sPref.getString("preference_zodiac_sign", "0"))]).userAgent(getResources().getString(R.string.user_agent)).timeout(getResources().getInteger(R.integer.user_timeout)).get();
                    	data = doc.getElementsByClass("date").get(0).text()+"<br /><br />"+doc.getElementsByClass("horoscope-content").get(0).child(0).text().replaceAll(doc.getElementsByClass("date").get(0).text()+" - ", "")+"<br /><br />"+getResources().getString(R.string.horoscope_com_copyright)+"<br />";
                    	dateLenght = Html.fromHtml(doc.getElementsByClass("date").get(0).text()).length()+1;
                    	if (!(doc.getElementsByClass("horoscope-content").get(0).child(0).text().replaceAll(doc.getElementsByClass("date").get(0).text()+" - ", "").length()<10))
                    		error=false;
