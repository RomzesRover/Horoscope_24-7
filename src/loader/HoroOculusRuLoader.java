...
public class HoroOculusRuLoader {
	...
	String[] resultS = new String[4];			//это текст html
	String[] horoscopes = new String[4];		//это просто текст для шаринга...
	
	boolean [] loaded = new boolean[4];
	
	String tag = "OculusRuLoader";
	
	int delayForThread;
	
	String[] months = {"Января", "Февраля","Марта", "Апреля", "Май", "Июня", "Июля", "Августа", "Сентября", "Октября", "Ноября", "Декабря"};
	
	public HoroOculusRuLoader (Context context, WebView[] webViews, RelativeLayout[] errorLayout, RelativeLayout[] allNormalLayout, ProgressBar[] progressBar ){
	        ...
	        public void run() {
	        	try {							//загрузка сегондя
					...
					resultS[0]="<span style=\"color: rgb(255, 69, 0);\"><span style=\"font-size: 18pt;\"><span style=\"font-size: 14pt;\"><span style=\"font-size: 16pt;\">"+
							...
							doc.getElementsByClass("oculus-epz-text").first().text().replaceFirst(context.getResources().getStringArray(R.array.zodiac_signs_ru)[zodiacNumber-1], "").replaceFirst(context.getResources().getStringArray(R.array.zodiac_signs_ru)[zodiacNumber-1], "")		//horo
							+"</span></span></span></p>";
					
					 horoscopes[0]=context.getResources().getStringArray(R.array.oculus_ru_horoscopes)[0]+" : #"+context.getResources().getStringArray(R.array.zodiac_signs)[zodiacNumber-1]+"\n\n"+doc.getElementsByClass("centerTitleBox").first().child(1).text()+"\n"+doc.getElementsByClass("oculus-epz-text").first().text().replaceFirst(context.getResources().getStringArray(R.array.zodiac_signs_ru)[zodiacNumber-1], "");
					setResult(0);
	...
	public void LoadTomorrow(){
		Thread thr=new Thread(new Runnable() {				//делаем в новом потоке
	        public void run() {
	        	try {							//загрузка сегондя
					Thread.sleep(delayForThread); 
					Document doc = Jsoup.connect("http://oculus.ru/"+context.getResources().getStringArray(R.array.nameOfHoroscopecForLoadOculusRu)[1]+"/"+context.getResources().getStringArray(R.array.nameOfzodiacForLoadOculusRu)[zodiacNumber-1]+".html").userAgent(context.getResources().getString(R.string.user_agent)).timeout(context.getResources().getInteger(R.integer.user_timeout)).get();
    				
    				resultS[1]="<span style=\"color: rgb(255, 69, 0);\"><span style=\"font-size: 18pt;\"><span style=\"font-size: 14pt;\"><span style=\"font-size: 16pt;\">"+
    						doc.getElementsByClass("centerTitleBox").first().child(1).text()				//date
    						+"</span></span></span></span></span></span></span></span><hr><p><span style=\"font-size: 14pt;\"><span style=\"font-size: 14pt;\"><span style=\"font-size: 14pt;\">"+
    						doc.getElementsByClass("oculus-epz-text").first().text().replaceFirst(context.getResources().getStringArray(R.array.zodiac_signs_ru)[zodiacNumber-1], "")		//horo
    						+"</span></span></span></p>";
    				
    				 horoscopes[1]=context.getResources().getStringArray(R.array.oculus_ru_horoscopes)[1]+" : #"+context.getResources().getStringArray(R.array.zodiac_signs)[zodiacNumber-1]+"\n\n"+doc.getElementsByClass("centerTitleBox").first().child(1).text()+"\n"+doc.getElementsByClass("oculus-epz-text").first().text().replaceFirst(context.getResources().getStringArray(R.array.zodiac_signs_ru)[zodiacNumber-1], "");
    				 setResult(1);
				} catch (NotFoundException e) {
					Log.e(tag, "data Error");
					// TODO Auto-generated catch block
		...
		Thread thr=new Thread(new Runnable() {				//делаем в новом потоке
	        public void run() {
	        	try {							//загрузка сегондя
					...
					Document doc=Jsoup.connect("http://oculus.ru/numerologiya/prognoz_na_den.php")		//юзаем куки
			        		.data("d",String.valueOf(sPref.getInt("dayBorn", 10)))				//тексто сообщения
			        		.data("m", String.valueOf(sPref.getInt("monthBorn",4)+1))				
			        		.data("d1", String.valueOf(c.get(Calendar.DAY_OF_MONTH)))
			        		.data("m1",String.valueOf(c.get(Calendar.MONTH)+1))
			        		.data("y1",String.valueOf(c.get(Calendar.YEAR)))
			        		.data("go", "")
			        		.post();									// отправляем
		    		...
		    		resultS[3]="<span style=\"color: rgb(255, 69, 0);\"><span style=\"font-size: 18pt;\"><span style=\"font-size: 14pt;\"><span style=\"font-size: 16pt;\">"+
		    						String.valueOf(c.get(Calendar.DAY_OF_MONTH))+" "+months[c.get(Calendar.MONTH)]+" "+c.get(Calendar.YEAR)				//date
		    						+"</span></span></span></span></span></span></span></span><hr><p><span style=\"font-size: 14pt;\"><span style=\"font-size: 14pt;\"><span style=\"font-size: 14pt;\">"+
		    						doc.getElementById("centerCol").child(5).html()		//horo
		    						+"</span></span></span></p>";
		    				
		    		horoscopes[3]=context.getResources().getStringArray(R.array.oculus_ru_horoscopes)[3]+" : #"+sPref.getString("name", context.getResources().getString(R.string.noname))+"\n\n"+String.valueOf(c.get(Calendar.DAY_OF_MONTH))+" "+months[c.get(Calendar.MONTH)]+" "+c.get(Calendar.YEAR)+"\n"+	doc.getElementById("centerCol").child(5).text()	;
		    		setResult(3);
				} catch (NotFoundException e) {
					Log.e(tag, "data Error");
					// TODO Auto-generated catch block
	...
	public void LoadWeek(){
		Thread thr=new Thread(new Runnable() {				//делаем в новом потоке
	        public void run() {
	        	try {							//загрузка сегондя
					Thread.sleep(delayForThread); 
					Document doc = Jsoup.connect("http://oculus.ru/"+context.getResources().getStringArray(R.array.nameOfHoroscopecForLoadOculusRu)[2]+"/"+context.getResources().getStringArray(R.array.nameOfzodiacForLoadOculusRu)[zodiacNumber-1]+".html").userAgent(context.getResources().getString(R.string.user_agent)).timeout(context.getResources().getInteger(R.integer.user_timeout)).get();
    				...
    				resultS[2]="<span style=\"color: rgb(255, 69, 0);\"><span style=\"font-size: 18pt;\"><span style=\"font-size: 14pt;\"><span style=\"font-size: 16pt;\">"+
    				 ...
    				 horoscopes[2]=context.getResources().getStringArray(R.array.oculus_ru_horoscopes)[2]+" : #"+context.getResources().getStringArray(R.array.zodiac_signs)[zodiacNumber-1]+"\n\n"+doc.getElementsByClass("centerTitleBox").first().child(1).text()+"\n"+doc.getElementsByClass("oculus-epz-text").first().text().replaceFirst(context.getResources().getStringArray(R.array.zodiac_signs_ru)[zodiacNumber-1], "");
    				 setResult(2);
				} catch (NotFoundException e) {
					Log.e(tag, "data Error");
					// TODO Auto-generated catch block
